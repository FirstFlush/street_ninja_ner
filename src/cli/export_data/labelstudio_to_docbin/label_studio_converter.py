...
class LabelStudioConverter:
    ...
    def convert_from_data(self, json: Any, split: DatasetSplit) -> Path:
        output_path = self.file_writer.output_path_from_split(split, self.OUTPUT_DIR, "json")
        converted_data = self._convert(json)
        self._save_json(converted_data, output_path)
        return output_path

    def convert_from_file(self, input_path: Path) -> Path:
    ...
    def _save_json(self, json: Any, output_path: Path):
        self.file_writer.save_json(output_path=output_path, json_data=json)
        logger.info(f"Saved {len(json)} records to {output_path}")
